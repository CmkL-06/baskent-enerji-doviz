...
const apiService = {
  ...
  voidVaultBalanceHistoryGroup: (historyIds: string[], reason: string) => post('/exchange/vault-balance-history/void-group', { historyIds, reason }),
  ...
  getExpensePendingApprovals:       (officeId?: any)            => get('/exchange/expense/payments/pending-approvals', { params: officeId ? { officeId } : {} }),
  approveExpensePayment:            (id: any, approve: boolean, rejectionNote?: string) => post(`/exchange/expense/payments/${id}/approve`, { approve, rejectionNote: rejectionNote ?? '' }),
  setExpenseBudget:                 (data: any)                 => post('/exchange/expense/budgets', data),
  getExpenseBudgetStatus:           (officeId: any, year: number, month: number) => get('/exchange/expense/budgets/status', { params: { officeId, year, month } }),
  getExpenseBudgetHistory:          (officeId: any, months = 12) => get('/exchange/expense/budgets/history', { params: { officeId, months } }),
  getUpcomingExpenseDefinitions:    (officeId: any)             => get('/exchange/expense/definitions/upcoming', { params: { officeId } }),
  getExpenseDefinitionStatement:    (definitionId: any)         => get(`/exchange/expense/definitions/${definitionId}/statement`),
  getExpenseCategories:             ()                          => get('/exchange/expense/categories'),
  saveExpenseCategory:              (data: any)                 => post('/exchange/expense/categories', data),
  deleteExpenseCategory:            (id: any)                   => post(`/exchange/expense/categories/delete/${id}`),

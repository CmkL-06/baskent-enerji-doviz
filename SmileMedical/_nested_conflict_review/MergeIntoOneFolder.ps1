...
$quantumYedekDir = Join-Path $desktop "QuantaQuokka_YEDEK\Quantum_Terminal"
...
if ($WhatIf) {
    ...
    if (Test-Path -LiteralPath $quantumYedekDir -PathType Container) {
        Get-ChildItem -LiteralPath $quantumYedekDir -File -ErrorAction SilentlyContinue | ForEach-Object {
            Write-Host "7. Tasinacak (YEDEK): $($_.FullName) -> $quantumTerminal\"
        }
    }
    Write-Host ""
...
Write-Host "Tasiniyor (Robocopy ile birlestiriliyor): QuantaQuokka_ -> SmileMedical"
# Robocopy /MOVE moves files and dirs, /E for recursive, /IS /IT to include same/tweaked files.
# /MOVE converts to /MOV for files and /MOVE for dirs.
# We use try/catch wrapper because robocopy sets weird exit codes (1 is success).
$p = Start-Process robocopy -ArgumentList "`"$sourceRoot`" `"$smileMedical`" /MOVE /E /IS /IT /NP /R:1 /W:1" -PassThru -NoNewWindow -Wait
if ($p.ExitCode -ge 8) {
    Write-Host "ROBOCOPY HATASI: Cikis kodu $($p.ExitCode)"
    exit 1
}

# Clean up source root if empty (Robocopy /MOVE might leave the root folder)
if (Test-Path -LiteralPath $sourceRoot -PathType Container) {
    ...
    if (-not $remaining) {
        ...
        Write-Host "UYARI: Kaynak klasor tamamen bos degil, silinmedi: $sourceRoot"
    }
}
...
if (Test-Path -LiteralPath $quantumYedekDir -PathType Container) {
    Get-ChildItem -LiteralPath $quantumYedekDir -File -ErrorAction SilentlyContinue | ForEach-Object {
        $dest = Join-Path $quantumTerminal $_.Name
        Move-Item -LiteralPath $_.FullName -Destination $dest -Force -ErrorAction SilentlyContinue
        Write-Host "Tasindi (YEDEK): $($_.Name) -> Quantum_Terminal\"
    }
}
